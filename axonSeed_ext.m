function [xi,yi] = axonSeed_ext(dist,N)
% [xi,yi] = axonSeed_ext(dist,N)
% 
% Description:
%   Wrapper for CUDA code to seed a set of spins onto only extra-axonal 
%   space in a distribution of myelinated axons
%
% inputs:
...
BlockMax = 256;
fun = parallel.gpu.CUDAKernel(fullfile('cu','mcSeed_ext.ptx'), ...
    fullfile('cu','mcSeed_ext.cu'));
...
[x] = feval(fun, x1, r1, r2, [dist.ro(:);dist.ri(:)], ...
    [dist.xr(:);dist.yr(:)], length(dist.yr(:)), dist.Lx, 0, N);
